...
class VideoStream:
	def __init__(self, filename):
		...
		try:
			...
			self.cache = bytearray(self.file.read(5))
			self.cacheFrameLen = (self.cache[0] - 48) * 10000 + (self.cache[1] - 48) * 1000 + (self.cache[2] - 48) * 100 + (self.cache[3] - 48) * 10 + (self.cache[4] - 48)
			self.cacheFrame = self.file.read(self.cacheFrameLen)
			if len(self.cacheFrame) != self.cacheFrameLen:
	...
	def testNextFrame(self):
		print ('-'*10 + "\nNext Frame (#" + str(self.frameNum) + ") length:" + str(self.cacheFrameLen) + "\n" + '-'*10)
		self.frameNum += 1
		return self.cacheFrame
